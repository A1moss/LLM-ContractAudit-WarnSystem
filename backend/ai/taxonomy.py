...
# 过渡方案：标准条款库(standard_clauses.json)内部仍按旧 5 类组织
# （采购/销售/保密协议/服务外包/劳动），后续按新框架补齐后删除本映射。
# 未列出的类别（服务外包/保密协议/劳动合同/无名合同）直接同名匹配标准条款库 type；
# 无名合同在标准条款库无对应 type，由 matcher 回退到全部条款比对。
TYPE_ALIAS: Dict[str, str] = {
    "买卖合同": "销售合同",          # 中性买卖 → 复用销售侧通用条款
    "技术合同": "服务外包合同",      # 技术开发/服务 → 服务类条款
    "承揽合同": "服务外包合同",      # 定制开发 → 服务类条款
    "委托合同": "服务外包合同",      # 委托 → 服务类条款
    "建设工程合同": "服务外包合同",  # 暂用服务类，待补建设类条款
    "中介合同": "服务外包合同",      # 居间 → 服务类条款
    "租赁合同": "销售合同",          # 暂用，待补租赁类条款
}
